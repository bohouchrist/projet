...
def sum_matrix(matrix):
    ...
    Args:
        matrix (list): Une matrice à deux dimensions (liste de listes).
...
tab = [[1, 2, 3, 4], [4, 5, 6], [7, 8, 9], [111, 222, 333]]
total_sum = sum_matrix(tab)
print("Somme de la matrice :", total_sum)


def transpose_matrix(matrix):
    """
    Transpose une matrice en échangeant les lignes et les colonnes.

    Args:
        matrix (list): Une matrice à deux dimensions (liste de listes).

    Returns:
        list: La matrice transposée.
    """
    nb_lignes = len(matrix)
    nb_colonnes = len(matrix[0])
    
    # Créez la matrice transposée en échangeant les lignes et les colonnes
    matrice_transposee = [[matrix[i][j] for i in range(nb_lignes)] for j in range(nb_colonnes)]
    
    return matrice_transposee

# Exemple d'utilisation :
matrice = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
matrice_transposee = transpose_matrix(matrice)

# Affichez la matrice originale
...
print("\nMatrice transposée :")
for row in matrice_transposee:
    print(row)


def rotate_matrix_90_degrees(matrix):
    """
    Effectue une rotation de 90 degrés dans le sens des aiguilles d'une montre sur une matrice.

    Args:
        matrix (list): Une matrice à deux dimensions (liste de listes).

    Returns:
        list: La matrice pivotée de 90 degrés.
    """
    if not matrix:
        return []
...
# Affichez la matrice rotatée de 90 degrés
print("\nMatrice rotatée de 90 degrés :")
